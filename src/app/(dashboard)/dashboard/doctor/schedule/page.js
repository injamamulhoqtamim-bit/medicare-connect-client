...
export default function DoctorSchedulePage() {
  ...
  const removeSchedule = async (scheduleId) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/doctor/schedule/${scheduleId}`, {
        method: "DELETE",
        credentials: "include",
      });
      ...
      setSchedules((current) =>
        current.filter((schedule) => schedule._id !== scheduleId)
      );
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <Link
        ...
        className="group flex items-center justify-between rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:border-teal-200 hover:shadow-md"
      >
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          <div>
            ...
            <select
              ...
              className="w-full rounded-lg border border-slate-300 px-3 py-2.5 outline-none focus:border-teal-500"
              disabled={adding}
            >
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 p-6">
          <h2 className="text-lg font-bold text-slate-800">Your Availability</h2>
          <p className="mt-1 text-sm text-slate-500">Current consultation schedule.</p>
        </div>
        ...
        <div className="divide-y divide-slate-100">
          {loading ? (
            ...
            <div className="p-8 text-center text-slate-500">No schedule added yet.</div>
          ) : (
            schedules.map((schedule) => (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-slate-800">{schedule.day}</h3>
                    <p className="text-sm text-slate-500">
                ...
                <button
                  ...
                  disabled={removingId === schedule._id}
                ...
                >
                  ...
                  {removingId === schedule._id ? "Removing..." : "Remove"}
                </button>
              </div>
            ))
          )}
