      ...
      <div>
        ...
        <div className="p-6">
          ...
          <div className="flex items-center gap-4 text-slate-500 text-sm mt-3">
            <span className="flex items-center gap-1"><Clock className="w-4 h-4" /> {doctor.experience}</span>
            <span className="flex items-center gap-1"><DollarSign className="w-4 h-4" /> ${doctor.fee}</span>
          </div>
        </div>
      </div>

      <div className="p-6 pt-0">
        <Link href={`/doctors/${doctor.id}`} className="btn bg-teal-600 hover:bg-teal-700 text-white w-full border-none rounded-xl">
          View Profile & Book
        </Link>
      </div>
    </div>
